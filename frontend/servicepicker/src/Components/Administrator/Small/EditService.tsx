...
import { getAllCategories, getServiceById } from '../../../Requests/GetRequests'
import { updateService } from '../../../Requests/PutRequests'
...
const EditService: React.FC<IEditService> = (props) => {
  const { classes, cx } = useStyles()

  const getData = async (isMounted: boolean) => {
    setLoading(true)
    const service = await getServiceById(props.id)
    ...
    if (isMounted) {
      setServiceData({
        ...
        categoryId: service.category?.id as string,
        url: service.url,
      ...
      setCategories(cats)
      setLoading(false)
      setPicUrl(`/servicePics/${service.picUrl}`)
    }
  }
